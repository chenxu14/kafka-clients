...
public class MockProducer<K, V> implements Producer<K, V> {
    ...
    private int partition(ProducerRecord<K, V> record, Cluster cluster) {
        ...
        ByteBuffer keyBytes = keySerializer.serialize(topic, record.headers(), record.key());
        ByteBuffer valueBytes = valueSerializer.serialize(topic, record.headers(), record.value());
